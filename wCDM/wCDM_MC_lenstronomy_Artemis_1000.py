...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("/project/Redshift_Difference/result/wCDM_chain_%d_2.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/wCDM_autocorr_%d_2.npy"%Nobs,autocorr)
        break
    old_tau = tau
