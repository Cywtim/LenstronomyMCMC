...
Nobs = 1000
zl_true,zs_true,Delta_z_true,Delta_z_obs = np.load("../data/wCDM%d-8.npy"%Nobs)
lens_kwargs = np.load("../data/wCDM%d-8_lens_parameters.npy"%Nobs)
...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("../result/wCDM_chain_%d.npy"%Nobs, sampler.flatchain)
        np.save("../Redshift_Difference/result/wCDM_autocorr_%d.npy"%Nobs,autocorr)
        break
    old_tau = tau
    
    if (sampler.iteration % 500)==0 and (sampler.iteration > 2000):
        np.save("../result/wCDM_chain_%d.npy"%Nobs, sampler.flatchain)
        np.save("../result/wCDM_autocorr_%d.npy"%Nobs,autocorr)
