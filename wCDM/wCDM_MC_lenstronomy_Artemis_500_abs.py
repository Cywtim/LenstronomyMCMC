...
def lnlike(p, zl, zs, Deltaz, z_err):
    ...
    lp = - 0.5 * sum( np.abs((Deltaz - model))/np.sqrt(denom) )
...
Nobs = 500
zl_true,zs_true,Delta_z_true,Delta_z_obs = np.load("/project/Redshift_Difference/data/wCDM%d-8.npy"%Nobs)
lens_kwargs = np.load("/project/Redshift_Difference/data/wCDM%d-8_lens_parameters.npy"%Nobs)
...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("/project/Redshift_Difference/result/wCDM_chain_%d_abs.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/wCDM_autocorr_%d_abs.npy"%Nobs,autocorr)
        break
    old_tau = tau
