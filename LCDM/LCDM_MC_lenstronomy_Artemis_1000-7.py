...
Nobs = 1000
e = 7
zl_true,zs_true,Delta_z_true,Delta_z_obs = np.load("/project/Redshift_Difference/data/LCDM%d-7.npy"%Nobs)
lens_kwargs = np.load("/project/Redshift_Difference/data/LCDM%d-7_lens_parameters.npy"%Nobs)
...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("/project/Redshift_Difference/result/LCDM_chain_%d-7.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/LCDM_autocorr_%d-7.npy"%Nobs,autocorr)
        break
    old_tau = tau   
