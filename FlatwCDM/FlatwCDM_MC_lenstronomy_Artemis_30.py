...
def lnlike(p, zl, zs, Deltaz, z_err):
    ...
    for i in range(len(zl)):
        ...
        temp = lens.NIE_redshifts(beta, lens_kwargs_list, cosmology_model,\
                zl_true[i], zs_true[i], [H0, Omega_M, 1-Omega_M, w0],\
...
Nobs = 30
zl_true,zs_true,Delta_z_true,Delta_z_obs = np.load("/project/Redshift_Difference/data/FlatwCDM%d-8.npy"%Nobs)
lens_kwargs = np.load("/project/Redshift_Difference/data/FlatwCDM%d-8_lens_parameters.npy"%Nobs)
...
p0 = np.array([[Omega_M_true, w0_true, H0_true ]])
...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("/project/Redshift_Difference/result/FlatwCDM_chain_%d.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/FlatwCDM_autocorr_%d.npy"%Nobs,autocorr)
        break
    old_tau = tau   
    
    if (sampler.iteration % 500)==0 and (sampler.iteration > 1000):
        np.save("/project/Redshift_Difference/result/FlatwCDM_chain_%d.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/FlatwCDM_autocorr_%d.npy"%Nobs,autocorr)
