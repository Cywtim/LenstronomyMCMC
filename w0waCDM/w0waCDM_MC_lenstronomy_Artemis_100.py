...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("/project/Redshift_Difference/result/w0waCDM_chain_%d.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/w0waCDM_autocorr_%d.npy"%Nobs,autocorr)
        break
    old_tau = tau
