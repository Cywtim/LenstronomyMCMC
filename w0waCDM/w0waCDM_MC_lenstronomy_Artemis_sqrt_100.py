...
def lnlike(p, zl, zs, Deltaz, z_err):
    ...
    lp = - 0.5 * np.sqrt(sum( np.power((Deltaz - model),2)/denom) + np.log(denom))
...
Nobs = 150
zl_true,zs_true,Delta_z_true,Delta_z_obs = np.load("/project/Redshift_Difference/data/w0waCDM%d-8.npy"%Nobs)
lens_kwargs = np.load("/project/Redshift_Difference/data/w0waCDM%d-8_lens_parameters.npy"%Nobs)
...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("/project/Redshift_Difference/result/w0waCDM_chain_sqrt_%d.npy"%Nobs, sampler.flatchain)
        np.save("/project/Redshift_Difference/result/w0waCDM_autocorr_sqrt_%d.npy"%Nobs,autocorr)
        break
    old_tau = tau
