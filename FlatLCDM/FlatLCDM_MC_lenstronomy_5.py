...
Nobs = 5
zl_true,zs_true,Delta_z_true,Delta_z_obs = np.load("../data/FlatLCDM%d-8.npy"%Nobs)
lens_kwargs = np.load("../data/FlatLCDM%d-8_lens_parameters.npy"%Nobs)
...
for sample in sampler.sample(p0, iterations=steps, progress=True, tune=True):
    ...
    if converged:
        np.save("../result/FlatLCDM_chain_%d.npy"%Nobs, sampler.flatchain)
        np.save("../result/FlatLCDM_autocorr_%d.npy"%Nobs,autocorr)
        break
    old_tau = tau   
